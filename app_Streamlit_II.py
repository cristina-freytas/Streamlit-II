...
def main():
    ...
    st.image(image, caption='Bank Branding', use_column_width=True)
    ...
    if (data_file_1 is not None):
        ...
        # Dados brutos
            ...
            plt.bar(bank_raw_target_perc.index, bank_raw_target_perc.values)
            plt.xlabel('Resposta')
            plt.ylabel('Porcentagem')
            plt.title('Dados brutos')
            plt.xticks(rotation=45)
            for i, v in enumerate(bank_raw_target_perc.values):
                plt.text(i, v + 0.5, f'{v:.1f}%', ha='center', va='bottom', rotation=45)
            st.pyplot()
            ...
            plt.figure(figsize=(8, 6))
            plt.bar(bank_target_perc.index, bank_target_perc.values)
            plt.xlabel('Resposta')
            plt.ylabel('Porcentagem')
            plt.title('Dados filtrados')
            plt.xticks(rotation=45)
            for i, v in enumerate(bank_target_perc.values):
                plt.text(i, v + 0.5, f'{v:.1f}%', ha='center', va='bottom', rotation=45)
            st.pyplot()
        else:
            st.write('### Dados brutos')
            bank_raw_target_perc = bank_raw.y.value_counts(normalize=True) * 100
            plt.figure(figsize=(8, 6))
            plt.pie(bank_raw_target_perc.values, labels=bank_raw_target_perc.index, autopct='%.1f%%', startangle=140)
            plt.title('Dados brutos', fontweight="bold")
            st.pyplot()
            ...
            bank_target_perc = bank.y.value_counts(normalize=True) * 100
            plt.figure(figsize=(8, 6))
            plt.pie(bank_target_perc.values, labels=bank_target_perc.index, autopct='%.1f%%', startangle=140)
            plt.title('Dados filtrados', fontweight="bold")
            st.pyplot()
